/**
 * Portable single-file AI Copilot “strategy / thesis / backtest” modal.
 *
 * Peer deps: react, react-dom, lucide-react, Tailwind CSS (project must support
 * arbitrary values and the Onest font if you want typography to match).
 *
 * Usage:
 *   const [open, setOpen] = useState(false);
 *   <AiCopilotThesisModal open={open} onOpenChange={setOpen} instrumentTitle="BTC/USDC" />
 */
...
export type AiCopilotThesisModalProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** e.g. vault / market name shown in the header */
  instrumentTitle?: string;
};

export function AiCopilotThesisModal({
  open,
  onOpenChange,
  instrumentTitle = "BTC/USDC",
}: AiCopilotThesisModalProps) {
  useEffect(() => {
    if (!open) return;
