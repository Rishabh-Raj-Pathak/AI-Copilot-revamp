...
export default function InstallAppButton({ page = "copilot", onClick }) {
  ...
  return (
    <button
      type="button"
      className={`install-app-prompt-btn fixed right-4 left-auto z-[60] hidden max-tablet:flex items-center rounded-full border border-transparent px-3.5 py-2 text-xs font-semibold text-[#f2b500] transition-[bottom,filter] duration-200 hover:brightness-110 ${page === "vaults" ? "" : bottomClass}`}
      style={vaultsStyle}
      onClick={onClick}
