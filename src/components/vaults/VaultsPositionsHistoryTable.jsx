...
import { VaultsVenueDropdown, VAULT_DEX_ICONS } from "./VaultsDexTabs.jsx";
...
function MobileStatRow({ label, value, valueClass = "text-[#d4d4d8]" }) {
  return (
    <div className="flex items-center justify-between gap-3">
      <span className="text-[10px] uppercase leading-[15px] tracking-[0.5px] text-[#717182]">
        {label}
      </span>
      <span className={`text-sm leading-5 ${valueClass}`}>{value}</span>
    </div>
  );
}

function PositionMobileCard({ row }) {
  return (
    <article className="vaults-root overflow-hidden rounded-[14px] border border-[rgba(255,255,255,0.06)] bg-gradient-to-b from-[#111116] to-[#0c0c10] p-4 shadow-[0_4px_20px_-5px_rgba(0,0,0,0.35)]">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          <span
            className={`flex size-6 shrink-0 items-center justify-center rounded-full text-[9px] font-semibold ${COIN_AVATAR[row.symbol] ?? "bg-white/10 text-white/80"}`}
            aria-hidden
          >
            {row.symbol.slice(0, 1)}
          </span>
          <span className="text-base font-semibold text-[#e8d5b5]">{row.symbol}</span>
        </div>
        <div className="flex flex-wrap justify-end gap-1">
          ...
          <span className="rounded border border-[rgba(196,168,112,0.16)] bg-[rgba(196,168,112,0.07)] px-[7px] py-0.5 text-[10px] leading-[15px] text-[rgba(196,168,112,0.85)]">
            {row.leverage}×
          </span>
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-col gap-2.5">
        <MobileStatRow label="Size" value={row.sizeLabel} />
        <MobileStatRow label="Pos. Value" value={row.posValue} valueClass="font-medium text-[#b0b0be]" />
        <MobileStatRow label="Entry Price" value={row.entryPrice} />
        <MobileStatRow label="Curr. Price" value={row.currPrice} valueClass="font-medium text-[#dcdce8]" />
        <MobileStatRow
          label="PNL"
          value={`${row.pnlUsd} ${row.pnlPct}`}
          valueClass={row.pnlPositive ? "font-medium text-[#00d492]" : "font-medium text-[#e5484d]"}
        />
        <MobileStatRow label="Liq. Price" value={row.liqPrice} />
        <MobileStatRow label="Margin" value={row.margin} valueClass="font-medium text-[#b0b0be]" />
        <MobileStatRow
          label="Funding"
          value={row.funding}
          valueClass={row.fundingPositive ? "font-medium text-[#00d492]" : "font-medium text-[#e5484d]"}
        />
        <MobileStatRow label="TP" value={row.tp} valueClass="text-[rgba(0,212,146,0.85)]" />
        <MobileStatRow label="SL" value={row.sl} valueClass="text-[rgba(229,72,77,0.85)]" />
        <MobileStatRow
          label="Exp. Profit"
          value={`${row.expProfit} ${row.expProfitPct}`}
          valueClass="text-[rgba(0,212,146,0.85)]"
        />
        <MobileStatRow
          label="Exp. Loss"
          value={`${row.expLoss} ${row.expLossPct}`}
          valueClass="text-[rgba(229,72,77,0.85)]"
        />
      </div>
...
function HistoryMobileCard({ row, venueLabel }) {
  return (
    <article className="vaults-root overflow-hidden rounded-[14px] border border-[rgba(255,255,255,0.06)] bg-gradient-to-b from-[#111116] to-[#0c0c10] p-4 shadow-[0_4px_20px_-5px_rgba(0,0,0,0.35)]">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[11px] text-[#717182]">{row.time}</p>
          <p className="mt-1 text-base font-semibold text-[#e8d5b5]">{row.symbol}</p>
        </div>
        <span
          className={`rounded px-[7px] py-0.5 text-[10px] leading-[15px] tracking-[0.3px] ${
            row.side === "long"
      ...
      <div className="mt-4 flex flex-col gap-2.5">
        <MobileStatRow label="Size" value={row.size} />
        <MobileStatRow label="Entry" value={row.entry} valueClass="text-[#b0b0be]" />
        <MobileStatRow label="Exit" value={row.exit} valueClass="font-medium text-[#dcdce8]" />
        <MobileStatRow
          label="PNL"
          value={row.pnl}
          valueClass={row.pnlPositive ? "font-medium text-[#00d492]" : "font-medium text-[#e5484d]"}
        />
        <MobileStatRow label="Venue" value={venueLabel(row.venue)} valueClass="uppercase text-[#717182]" />
        <MobileStatRow label="Status" value={row.status} valueClass="text-[#b0b0be]" />
      </div>
    </article>
  );
}
...
export default function VaultsPositionsHistoryTable() {
  ...
  return (
    ...
    >
      <div className="rounded-[24px] border border-[rgba(255,255,255,0.06)] bg-[#050505] px-4 pb-5 pt-6 max-tablet:rounded-[16px] sm:px-8 tablet:px-6 tablet:pb-6 tablet:pt-8">
        ...
        <div className="mt-6 max-tablet:block tablet:hidden">
          <VaultsVenueDropdown
            tabs={dexTabs}
            activeId={venueId}
            onChange={setVenueId}
            ariaLabel="Venue filters for positions table"
          />
        </div>

        <div className="mt-8 hidden min-w-0 max-w-full overflow-x-auto pb-0.5 [scrollbar-width:thin] tablet:block">
          <div
          ...
          >
            {dexTabs.map((tab) => {
              const Icon = VAULT_DEX_ICONS[tab.id];
              ...
              return (
                ...
                >
                  {Icon ? <Icon className={`size-4 shrink-0 ${active ? "text-[#e8d5b5]" : "text-[#717182]"}`} aria-hidden /> : null}
                  {tab.label}
                </button>
              );
            })}
        ...
        {mainTab === "positions" ? (
          <>
            ...
            <div className="mt-6 flex flex-col gap-3 max-tablet:flex tablet:hidden">
              {filteredPositions.map((row) => (
                <PositionMobileCard key={row.id} row={row} />
              ))}
              {filteredPositions.length === 0 ? (
                <p className="rounded-xl border border-[rgba(255,255,255,0.05)] bg-[#0c0c0c] px-4 py-6 text-center text-sm text-[#717182]">
                  No open positions for this venue.
                </p>
              ) : null}
            </div>

            {/* Desktop positions grid */}
            <div className="mt-8 hidden min-w-0 tablet:block">
          ...
          <>
            ...
            <div className="mt-6 flex flex-col gap-3 max-tablet:flex tablet:hidden">
              {filteredHistory.map((row) => (
                <HistoryMobileCard key={row.id} row={row} venueLabel={venueLabel} />
              ))}
