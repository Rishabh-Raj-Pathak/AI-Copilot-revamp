...
/**
 * The second half of step three: share the first closed trade's PnL card.
 *
 ...
 *
 * This action is shown from the closed-trade flow, after a position has been
 * opened and closed, so the CTA can refer to "this trade" without repeating a
 * synthetic preview. The composer is opened from the click handler, never from
 * the timer: a popup blocker eats anything a user gesture didn't open. The step
 * then lands on the post coming back, the same way the link above it lands on
 * its callback.
 *
 * @param {object} props
...
export default function SharePnlStep({ account, shared, onShared, onNotify }) {
    ...
    return (
      <div className="flex items-center gap-2.5 rounded-lg border border-[#1e5a3f] bg-[#0d2019] px-3 py-2">
        ...
        <span className="min-w-0 flex-1 truncate text-sm font-medium text-white">
          Shared from {account?.handle ?? "your account"}
        </span>
  ...
  const post = () => {
    ...
    cancelRef.current = startPnlCardShare(() => {
      ...
      setPending(false);
      onShared();
      onNotify?.(`PnL shared to X — +${X_SHARE_POINTS} pts earned`, "success");
    });
  };

  return (
    /* The settled X row sits immediately above this action. The trade result
       itself is already visible when the action is offered, so duplicating its
       figures here would turn a simple next step into a second preview. */
    <button
      type="button"
      onClick={post}
      disabled={pending}
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 flex-col">
        <span className="text-sm font-semibold text-white">
          {pending ? "Sharing to X…" : "Share this trade’s PnL"}
        </span>
        <span className="text-xs leading-snug text-[#929292] max-sm:line-clamp-2 sm:truncate">
          Share on X as {account.handle}. Your referral link is included.
        </span>
      </span>
